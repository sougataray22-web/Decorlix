...
const registerUser = asyncHandler(async (req, res) => {
  ...
  if (existingUser) { res.status(409); throw new Error("An account with this email already exists."); }
  const user = await User.create({ name: name.trim(), email: email.toLowerCase().trim(), password, isVerified: true });
...
const loginUser = asyncHandler(async (req, res) => {
  ...
  if (!user.password) { res.status(400); throw new Error("This account uses phone login. Please sign in with OTP."); }
...
const changePassword = asyncHandler(async (req, res) => {
  ...
  if (!user.password) { res.status(400); throw new Error("This account uses phone login."); }
...
const firebaseLogin = asyncHandler(async (req, res) => {
  ...
  if (!idToken) { res.status(400); throw new Error("Firebase ID token is required."); }
  const admin = initFirebase();
  let decodedToken;
  try {
    decodedToken = await admin.auth().verifyIdToken(idToken);
  } catch (error) {
    res.status(401); throw new Error("Invalid or expired OTP. Please try again.");
  }
  const { uid, phone_number } = decodedToken;
  if (!phone_number) { res.status(400); throw new Error("Phone number not found in token."); }
  ...
  const newUser = await User.create({
    ...
    phone: phone10, firebaseUid: uid, isVerified: true, role: "user",
  });
...
const adminLogin = asyncHandler(async (req, res) => {
  ...
  if (!email || !password) { res.status(400); throw new Error("Please provide email and password."); }
  const user = await User.findOne({ email: email.toLowerCase() }).select("+password");
  if (!user || user.role !== "admin") { res.status(401); throw new Error("Invalid credentials or insufficient permissions."); }
  const isMatch = await user.matchPassword(password);
  if (!isMatch) { res.status(401); throw new Error("Invalid credentials."); }
  sendTokenResponse(user, 200, res, "Admin login successful.");
});

module.exports = { registerUser, loginUser, getMe, updateProfile, changePassword, firebaseLogin, adminLogin };
